...
"""
Important:
1. Please pass the correct data_address, grid_address, nod2dfile in slurm script.
2. Please set nc_flag to 0 if you do not have .nc file for grid
3. Please change the resolution value in region_init function.
4. Please change the out_path to your preffered location in to_netcdf function.
...
class Interpolator:
    def __init__(self, data_address, grid_address, nod2dfile, elm2dfile, year, month, left, right, top, bottom, nc_flag: int = 1):
        """
        Initilizer function
        arguments:
        -----------
        ...
        if self.nc_flag:
            ...
            self.file_content = pd.read_csv(
            nod2dfile,
            delim_whitespace=True,
            skiprows=1,
            names=["node_number", "x", "y", "flag"],
            )
            self.model_lon = self.file_content.x.values
            self.model_lat = self.file_content.y.values
    ...
    def to_netcdf(self, linear_interpolator, days: int = 30) -> None:
        """
        ...
        for day in range(days): 
            ...
            t=ts.to_pydatetime()
            ssh[day] = linear_interpolator[day][:]
            time[day] = date2num(t, time_unit_out)
        
        fw.close()
        return None     
        
    def linear_interpolation_action(self):
        """
        ...
        print("1122")
        self.to_netcdf(linear_interpolator_list, days)
        
        
    def nn_interpolation_action(self, mask_flag: int = 0):
        """
        ...
        self.to_netcdf(nn_interpolation, days)
